...
async def _search_async(book_name: str):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            results = await _extract_results_from_rows(
                page, row_locator, selector_used, debug_log, context
            )
...
async def _extract_results_from_rows(
    page, rows, selector_used, debug_log, context=None
):
...
async def _extract_hierarchical(page, platforms, limit, debug_log, context):
    ...
    for p_idx in range(min(platform_count, limit)):
        ...
        try:
            ...
            if await head.count() > 0:
                platform_name = _clean_text(await head.inner_text())
        except Exception:
            pass
        ...
        for b_idx in range(min(book_count, 20)):
            ...
            try:
                ...
                if await acc_title.count() > 0:
                    book_title = _clean_text(await acc_title.inner_text())
            except Exception:
                pass
    ...
    if context and tasks_to_run:
        ...
        for t in tasks_to_run:
            if t["pid"] not in platform_firsts:
                platform_firsts[t["pid"]] = t
        ...
        platform_metas = {
            pid: (m if not isinstance(m, Exception) else {})
            for pid, m in zip(platform_firsts.keys(), metadatas)
        }
    else:
        platform_metas = {}
    ...
    for t in tasks_to_run:
        ...
        if t["pid"] == 3 and meta.get("title"):
            if any(g in meta["title"] for g in ["評論", "AI之眼", "AI世代", "Python"]):
                if len(t["btitle"]) > len(meta["title"]):
                    final_title = t["btitle"]
...
async def _deep_crawl_for_metadata(url, context, debug_log, platform_hint=None):
    ...
    try:
        ...
        if platform_hint == 2:
            # UDN uses Nuxt.js - extract metadata from window.__NUXT__ data
            ...
            try:
                nuxt_data = await page.evaluate("""
                    () => {
                        try {
                            ...
                            // Try to find script containing __NUXT__
                            const scripts = document.querySelectorAll('script');
                            for (const s of scripts) {
                                if (s.textContent && s.textContent.includes('__NUXT__')) {
                                    const match = s.textContent.match(/__NUXT__\\s*=\\s*({.+?});/s);
                ...
                """)
                if nuxt_data and nuxt_data.get("productDetail"):
                    pd = nuxt_data["productDetail"]
                    if pd.get("data"):
                        metadata["title"] = pd["data"].get("title", "")
                        metadata["author"] = pd["data"].get("author", "")
                        metadata["date"] = pd["data"].get("publishdate", "")
                        metadata["format"] = pd["data"].get("format", "")
                        metadata["frontcover"] = pd["data"].get("cover", "")
                        await _log(debug_log, "Extracted from __NUXT__")
                        return metadata
            except Exception as e:
                await _log(debug_log, f"UDN Nuxt extract failed: {e}")
            # Fallback: wait for DOM
            try:
                await page.wait_for_selector(
                    ".loading, .spinner, [class*='loading'], .el-loading",
                    state="hidden",
                    timeout=15000,
                )
                await page.wait_for_selector(
                ...
                await _log(debug_log, f"UDN wait fallback failed: {e}")
        elif platform_hint == 3:
            try:
            ...
            try:
                await page.wait_for_selector(".detail h2", timeout=10000)
            except Exception:
                pass
...
async def _extract_detail_field(page, field_type, platform_hint=None):
    try:
        if field_type == "title":
            ...
            if platform_hint == 1:
                el = page.locator(
                    "p.note:has-text('作者'), #center p:has-text('作者')"
                ).first
                if await el.count() > 0:
                    return _clean_metadata(await el.inner_text())
            if platform_hint == 3:
                el = page.locator(
                    "#bookdetailcpcontentblock li:has-text('作者：') a, #bookdetailcpcontentblock li:has-text('作者：')"
                ).first
                if await el.count() > 0:
                    return _clean_metadata(await el.inner_text())
            if platform_hint == 4:
                el = page.locator(
                    "td:has-text('作者') + td, .detail-info li:has-text('作者')"
                ).first
                if await el.count() > 0:
                    return _clean_metadata(await el.inner_text())
            ...
            m = re.search(r"作者[:：\s]*([^\n|｜\s]+)", body)
            if m:
                return _clean_metadata(m.group(1))

        elif field_type == "date":
            if platform_hint == 3:
                try:
                    labels = page.locator("#bookdetailcpcontentblock li").filter(
                        has_text=re.compile(r"出版年：|日期：")
                    )
                    for i in range(await labels.count()):
                        ...
                        if m2:
                            return str(1911 + int(m2.group(1)))
                except Exception:
                    pass
            elif platform_hint == 2:
                try:
                    # Fallback to searching all p.text-secondary for date patterns
                    # to avoid encoding issues with headings
                    p_elements = page.locator("p.text-secondary")
                    for i in range(await p_elements.count()):
                        text = (await p_elements.nth(i).inner_text()).strip()
                        # Match YYYY-MM-DD
            ...
            body = await page.inner_text("body")
            m = re.search(
                r"出版日期[:：\s]*(?!線上)(19|20)\d{2}[/-]\d{1,2}[/-]\d{1,2}", body
            )
            if m:
                return m.group(0)
            ...
            selectors = [
                ".book-cover img",
                "#bookdetailcpcontentblock figure img",
                ".cover img",
                "img[alt='cover']",
            ]
            for s in selectors:
                try:
                    el = page.locator(s).first
                    if await el.count() > 0:
                        src = await el.get_attribute("src") or await el.get_attribute(
                            "data-original"
                        )
                        if (
                            src
...
def _normalize_date(d):
    ...
    m = re.search(
        r"((?:19|20)\d{2})([-/.])(1[0-2]|0?[1-9])\2([12][0-9]|3[01]|0?[1-9])", d
    )
    if m:
